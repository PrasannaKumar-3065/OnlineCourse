...
<head>
    ...
    <title>HOD | Credit Transfer Approved</title>
...
<?php
$sql = mysqli_query($bd,"select * from students a inner join noncgpa b on a.StudentRegno = b.name where b.type= 'Credit Transfer' and b.status = 'Approved_by_HOD' and a.batch = '" .$reg. "' ");
$cnt=1;
$credit = 0;
$sem = 0;
...
?>
                                        <tr>
                                            ...
                                            <?php 
                                            $color = ""; if($row["status"] == "Approved_by_HOD"){$color = "text-success";}else{$color = "text-danger";} ?>
...
$cnt++;
    ...
    echo '<script>alert("No Credit Transfers are approved for '.$reg.' batch ");</script>';
}?>
